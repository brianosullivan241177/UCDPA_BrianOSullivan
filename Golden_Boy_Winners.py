...
FifaAll_Players_DF = pd.read_csv('/users/brian/documents/FIFA_Project_Final/Modified/FIFAAllplayers_GB.csv',
                           usecols=['sofifa_id','short_name','league_name','club_name','wage_eur', 'overall','age','preferred_foot',
                                    'Season', 'height_cm','nationality_name','potential'], index_col=[0])
print(FifaAll_Players_DF.head()) #top records
print(FifaAll_Players_DF.dtypes) #dataframe data tyoes

FifaAll_Players_DF.rename(columns = {'overall':'Overall Rating in %', 'wage_eur':'Wages paid in Euro',
                                     'short_name':'Player_name'}, inplace = True)

Golden_Boy_Winners_DF = FifaAll_Players_DF[
    (FifaAll_Players_DF.Player_name == 'GB Winner 2010 M. Balotelli')
    |(FifaAll_Players_DF.Player_name == 'GB Winner 2011 M. Götze')
    |(FifaAll_Players_DF.Player_name == 'GB Winner 2012 Isco')
    |(FifaAll_Players_DF.Player_name == 'GB Winner 2013 P. Pogba')
    |(FifaAll_Players_DF.Player_name == 'GB Winner 2014 R. Sterling')
]
...
a = sns.catplot(data=Golden_Boy_Winners_DF, x='Season',
                   y='Overall Rating in %', hue='Player_name', height=6, aspect=2, order=plot_order, kind="point")
...
a1 = sns.catplot(data=Golden_Boy_Winners_DF, x='Season',
                   y='Wages paid in Euro', hue='Player_name', height=6, aspect=2, order=plot_order, kind="point")
