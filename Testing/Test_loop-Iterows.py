...
Fifa_DF = pd.read_csv("/Users/brian/Documents/FIFA_Project/Modified/players_20_21.csv",
...
Fifa_All_DF = pd.read_csv("/Users/brian/Documents/FIFA_Project/Modified/Allplayers.csv",
                   usecols=['short_name', 'height_cm', 'overall', 'potential','age','work_rate','Season','league_name','club_name','wage_eur'])

Coleman_DF = Fifa_All_DF[(Fifa_All_DF.short_name == 'S. Coleman')]
print(Coleman_DF)
Coleman_Sort_DF = Coleman_DF.sort_values(by=['Season', 'potential','wage_eur'], inplace=True, ascending=True)
for index, row in Coleman_DF.iterrows():
    print(index, ': ', row['short_name'], 'had the potential of',row['potential'],'and wages of',row['wage_eur'],'in season',row['Season'] )
